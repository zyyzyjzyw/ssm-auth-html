  ...
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="状态" width="80">
        <template slot-scope="scope">
          <el-switch
            v-model="scope.row.status === 1" :disabled="scope.row.children.length > 0"
      ...
      <el-table-column label="操作" width="180" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button type="success" v-if="scope.row.type !== 2" icon="el-icon-plus" size="mini" :disabled="$hasBP('bnt.sysMenu.add')  === false" @click="add(scope.row)" title="添加下级节点"/>
          <el-button type="primary" icon="el-icon-edit" size="mini" :disabled="$hasBP('bnt.sysMenu.update')  === false" @click="edit(scope.row)" title="修改"/>
          <el-button type="danger" icon="el-icon-delete" size="mini" @click="removeDataById(scope.row.id)" title="删除" :disabled="$hasBP('bnt.sysMenu.remove')  === false"/>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    switchStatus(row){
      ...
      api.updateMenuStatus(row.id,row.status).then(response=>{
          //提示
          this.$message({
                type: 'success',
                message: '修改成功!'
